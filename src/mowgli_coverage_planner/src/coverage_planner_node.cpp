...
: rclcpp::Node("coverage_planner_node", options)
{
  ...
  RCLCPP_INFO(
    ...
    tool_width_, headland_passes_, headland_width_, path_spacing_,
    decompose_cells_ ? "true" : "false", map_frame_.c_str());
  ...
  action_server_ = rclcpp_action::create_server<PlanCoverageAction>(
    this,
    "~/plan_coverage",
    std::bind(&CoveragePlannerNode::handle_goal, this,
...
static void publish_feedback_impl(
  ...
  float progress, const std::string & phase)
{
  ...
  const std::shared_ptr<GoalHandlePlanCoverage> goal_handle)
{
    ...
    try {
      for (int ci = 0; ci < static_cast<int>(work_cells.size()); ++ci) {
        ...
        if (cell_geom->getGeometryType() == wkbPolygon) {
          result_cells.addGeometry(
            ogr_poly_to_f2c(static_cast<const OGRPolygon *>(cell_geom)));
          ...
          for (int gi = 0; gi < mp->getNumGeometries(); ++gi) {
            result_cells.addGeometry(
              ogr_poly_to_f2c(static_cast<const OGRPolygon *>(
                mp->getGeometryRef(gi))));
          }
        }
        OGRGeometryFactory::destroyGeometry(cell_geom);
      ...
      for (int wci = 0; wci < static_cast<int>(work_cells.size()); ++wci) {
        RCLCPP_INFO(get_logger(),
          ...
          wci, work_cells.getGeometry(wci).size(),
          work_cells.getGeometry(wci).size() > 0 ?
          work_cells.getGeometry(wci).size() - 1 : 0);
      }
      RCLCPP_INFO(get_logger(),
        "F2C: subtracted %zu obstacles -> %d work cell(s)",
        num_obstacle_rings, static_cast<int>(work_cells.size()));
    } catch (const std::exception & ex) {
  ...
  try {
    for (int ci = 0; ci < static_cast<int>(work_cells.size()); ++ci) {
      ...
      if (auto_angle) {
        cell_swaths = swath_gen.generateBestSwaths(
          n_swath_obj, path_spacing_, work_cells.getGeometry(ci));
      } else {
        cell_swaths = swath_gen.generateSwaths(
          swath_angle_rad, path_spacing_, work_cells.getGeometry(ci));
      }
  ...
  RCLCPP_INFO(get_logger(), "F2C: %zu swaths across %zu sub-cell(s)",
    total_swaths, all_swaths.size());
  ...
  RCLCPP_INFO(get_logger(),
    ...
    swath_path.poses.size(), swath_starts.size(), f2c_path.length(),
    min_turning_radius_);
  ...
  RCLCPP_INFO(get_logger(),
    ...
    result->total_distance, result->coverage_area);
...
nav_msgs::msg::Path CoveragePlannerNode::generate_outline_path(
  const Polygon2D & outer, const std::string & frame) const
{
...
CoveragePlannerNode::extract_costmap_obstacles() const
{
  ...
  for (int sy = 0; sy < height; ++sy) {
    for (int sx = 0; sx < width; ++sx) {
      ...
      p.x = static_cast<float>(min_x); p.y = static_cast<float>(min_y); poly.points.push_back(p);
      p.x = static_cast<float>(max_x); p.y = static_cast<float>(min_y); poly.points.push_back(p);
      p.x = static_cast<float>(max_x); p.y = static_cast<float>(max_y); poly.points.push_back(p);
      p.x = static_cast<float>(min_x); p.y = static_cast<float>(max_y); poly.points.push_back(p);
