...
void WheelOdometryNode::on_wheel_tick(mowgli_interfaces::msg::WheelTick::ConstSharedPtr msg)
{
  ...
  // ------------------------------------------------------------------
  const auto ticks_delta = [](uint32_t current, uint32_t previous,
                               uint8_t dir) -> double {
  ...
  const double delta_ticks_left  = ticks_delta(
    msg->wheel_ticks_rl, prev_ticks_left_,  msg->wheel_direction_rl);
  const double delta_ticks_right = ticks_delta(
    msg->wheel_ticks_rr, prev_ticks_right_, msg->wheel_direction_rr);
