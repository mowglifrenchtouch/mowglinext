...
: Node("diagnostics_node", options)
{
  ...
  RCLCPP_INFO(
    ...
    publish_rate_, freshness_warn_sec_, freshness_error_sec_);
}
...
void DiagnosticsNode::create_subscriptions()
{
  ...
  sub_gps_ = create_subscription<sensor_msgs::msg::NavSatFix>(
    "/gps/fix", sensor_qos,
    [this](sensor_msgs::msg::NavSatFix::ConstSharedPtr msg) { on_gps(msg); });
}
...
void DiagnosticsNode::create_timer()
{
  ...
  timer_ = create_wall_timer(period_ms, [this]() { publish_diagnostics(); });
}
  ...
  const rclcpp::Time & now) const
{
  ...
  const double age_sec =
    (now - state_.last_status_time).seconds();

  status.level   = classify_freshness(
    age_sec, false, freshness_warn_sec_, freshness_error_sec_);
  status.message = "Last status age: " + fmt_float(age_sec, 1) + "s";

  status.values.push_back(kv("age_sec", fmt_float(age_sec, 2)));
  status.values.push_back(kv(
      "mower_status",
      std::to_string(state_.last_status->mower_status)));
  status.values.push_back(kv(
      "is_charging",
      state_.last_status->is_charging ? "true" : "false"));
  status.values.push_back(kv(
      "mow_enabled",
      state_.last_status->mow_enabled ? "true" : "false"));
...
diagnostic_msgs::msg::DiagnosticStatus DiagnosticsNode::check_battery() const
{
  ...
  status.values.push_back(kv("charge_current_a", fmt_float(static_cast<double>(p.charge_current), 3)));
...
diagnostic_msgs::msg::DiagnosticStatus DiagnosticsNode::check_imu(
  const rclcpp::Time & now) const
{
  diagnostic_msgs::msg::DiagnosticStatus status;
  status.name        = "IMU";
...
diagnostic_msgs::msg::DiagnosticStatus DiagnosticsNode::check_lidar(
  const rclcpp::Time & now) const
{
  diagnostic_msgs::msg::DiagnosticStatus status;
  status.name        = "LiDAR";
...
diagnostic_msgs::msg::DiagnosticStatus DiagnosticsNode::check_gps(
  const rclcpp::Time & now) const
{
  diagnostic_msgs::msg::DiagnosticStatus status;
  status.name        = "GPS";
  ...
  if (!has_fix) {
    ...
    status.level = classify_freshness(age_sec, false, freshness_warn_sec_, freshness_error_sec_);
    status.message = "GPS fix OK  lat=" + fmt_float(gps.latitude, 6) +
      "  lon=" + fmt_float(gps.longitude, 6);
  }
...
diagnostic_msgs::msg::DiagnosticStatus DiagnosticsNode::check_motors() const
{
  ...
  const uint8_t motor_level = classify_temperature(motor_temp, motor_temp_warn_c_, motor_temp_error_c_);
  ...
  status.values.push_back(kv("mower_esc_current_a",  fmt_float(static_cast<double>(s.mower_esc_current), 2)));
...
diagnostic_msgs::msg::KeyValue DiagnosticsNode::kv(
