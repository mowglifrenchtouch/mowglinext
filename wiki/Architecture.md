...
| Layer | Owner | Notes |
|---|---|---|
| `map → odom` | static identity transform | Published once at launch; the odom frame IS the GPS-ENU frame. No SLAM. |
| `odom → base_footprint` | FusionCore UKF @ 100 Hz | 22D quaternion state; fuses GPS, IMU, wheels, and optionally Kinematic-ICP twist on `/encoder2/odom`. IMU + GPS antenna lever arms auto-resolved from URDF TF. |
| Kinematic-ICP | parallel TF tree | Reads `/scan` via a scan-frame relay onto `wheel_odom_raw → base_footprint_wheels → lidar_link_wheels`. Output feeds FusionCore only through the encoder2 adapter — no feedback into the main TF. |
...
FusionCore node (100 Hz, single 22D quaternion UKF — sole localizer):
   - Fuses: /wheel_odom + /imu/data + /gps/fix (NavSatFix directly)
             + /encoder2/odom (optional Kinematic-ICP twist)
   - Applies IMU + GPS antenna lever arms (auto-resolved from URDF TF).
   - Output: /fusion/odom (nav_msgs/Odometry)
   - Publishes TF: odom → base_footprint

↓

Static identity publisher (tf2_ros static_transform_publisher, at launch):
   - Publishes TF: map → odom = identity, once.
   - `map` is the same frame as `odom` — the GPS-ENU frame.
...
Kinematic-ICP (on a parallel TF tree — never part of the main robot tree):
   ...
   - kinematic_icp_encoder_adapter finite-differences the pose into a
     body-frame twist and publishes /encoder2/odom, which FusionCore's
     UKF consumes through the encoder2 callback.

↓

Final Output:
  /tf tree: map → odom → base_footprint
  /fusion/odom (fused state estimate)
...
```
Map frame (== odom; GPS-ENU, RTK-anchored)
    │
    ├── [static_transform_publisher emits map→odom = identity once at launch]
    │
Odometry frame
    │
    ├── [FusionCore publishes odom→base_footprint @ 100 Hz]
    │
...
**navigation.launch.py** – FusionCore + Nav2 stack (included by main)

Starts (in order):
1. `static_transform_publisher` publishing `map → odom = identity` once.
2. `fusioncore_node` (LifecycleNode, auto-configured + activated). Sole
   publisher of `odom → base_footprint`. Gated on `use_ekf`.
...
**localization.yaml** – FusionCore UKF tuning: IMU noise, GPS covariance, ZUPT, adaptive R, lever arms, `apply_lever_arm_pre_heading`.
...
5. Feedback loop (real-time):
   ...
   FusionCore (single 22D UKF, 100 Hz):
     ├─ Fuses /wheel_odom + /imu/data + /gps/fix (NavSatFix directly)
     │      + /encoder2/odom (optional Kinematic-ICP twist)
     ├─ Applies IMU + GPS lever arms (URDF-resolved)
   ...
   Kinematic-ICP (optional, 10 Hz, parallel TF tree):
     ...
     └─→ adapter → /encoder2/odom  (fed back into FusionCore's UKF)

   map → odom is a STATIC IDENTITY transform, published once at launch.
...
11. Telemetry (Foxglove Bridge, 8765/ws):
    → Web UI receives:
       ├─ /fusion/odom (fused pose + covariance)
...
```
map (== odom; GPS-ENU, RTK-anchored)
  │ [static_transform_publisher emits map→odom = identity once at launch]
  │
  odom
  │ [published by FusionCore @ 100 Hz]
  │
  ...
  │        FusionCore rotates gyro+accel into base_link and applies the
  │        lever-arm centripetal correction]
  │
  ...
  │   lever arm from base_footprint auto-resolved from TF by FusionCore)
  │
...
|-------|-----------|------|---------|
| map | static_transform_publisher | once, at launch | Identity with `odom`. No SLAM: the GPS-ENU datum is the global origin. |
| odom | FusionCore | 100 Hz | GPS-ENU parent of `base_footprint` — RTK-anchored via FusionCore's UKF |
| base_footprint | FusionCore | 100 Hz | On ground, robot frame for Nav2/controllers (REP-105) |
| base_link | robot_state_publisher | Static | Rear wheel axle (OpenMower convention) |
| imu_link | robot_state_publisher | Static | IMU sensor frame (FusionCore reads lever arm via URDF) |
| lidar_link | robot_state_publisher | Static | LiDAR origin (collision_monitor + obstacle_layer consume here) |
| gps_link | robot_state_publisher | Static | GPS antenna (FusionCore reads lever arm via URDF) |
...
- `map` – Global frame, z-up, x-east, y-north (REP-103). Equal to `odom` (static identity).
- `odom` – GPS-ENU frame, anchored to the first RTK-grade fix (`reference.use_first_fix: true`).
...
| `/wheel_odom` | nav_msgs/Odometry | hardware_bridge_node | ~10 Hz | Integrated wheel velocities (RELIABLE to match FusionCore's subscription) |
...
| `/fusion/odom` | nav_msgs/Odometry | FusionCore | 100 Hz | Fused state (odom→base_footprint) |
...
| `/encoder2/odom` | nav_msgs/Odometry | kinematic_icp_encoder_adapter | 10 Hz | Finite-differenced K-ICP twist, consumed by FusionCore's UKF |
| `/localization/status` | diagnostic_msgs/DiagnosticStatus | localization_monitor_node | 2 Hz | GPS quality + FusionCore health |
...
| `/imu/data` | FusionCore, diagnostics_node | Sensor fusion + monitoring |
| `/fusion/odom` | nav2 controller, BT, diagnostics_node, GUI | Localized pose for control, behavior tree, diagnostics |
| `/map` | nav2 planner, behavior_tree (area polygons), diagnostics_node | Global navigation reference (area-polygon grid) |
| `/gps/fix` | FusionCore, navsat_to_absolute_pose_node, diagnostics_node | Sensor fusion, convert fix to local frame, monitor GPS |
| `/encoder2/odom` | FusionCore | K-ICP twist consumed as a secondary encoder |
...
5. **Single-Tier UKF Localization (FusionCore — sole localizer):**
   - Fuses wheel odometry + IMU + GPS (+ optional Kinematic-ICP twist) in a
     single 22D quaternion UKF at 100 Hz.
   - Outputs `/fusion/odom` (odom→base_footprint) with RTK-anchored position.
   - `map → odom` is a static identity transform — no SLAM back-end.
     The GPS-ENU frame IS the map frame, pinned to the first RTK fix.
   - IMU + GPS antenna lever arms are auto-resolved from URDF TF (no
     duplicate bookkeeping in YAML), and `apply_lever_arm_pre_heading`
     makes GPS constrain yaw from fix #1.
   - Graceful degradation: operates without GPS in GNSS-denied areas
     (wheel + IMU + Kinematic-ICP when LiDAR is present).
...
| FusionCore Config | src/mowgli_bringup/config/localization.yaml | UKF noise, GPS covariance, ZUPT, adaptive R, lever arms |
