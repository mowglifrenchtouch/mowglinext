...
{
  return starts_with(line, "ICY 200") ||
         starts_with(line, "HTTP/1.0 200") ||
         starts_with(line, "HTTP/1.1 200");
}

bool is_textual_error_response(const std::string & text)
{
  return starts_with(text, "SOURCETABLE") ||
         starts_with(text, "ERROR") ||
         starts_with(text, "ICY 401") ||
         starts_with(text, "ICY 403") ||
         starts_with(text, "ICY 404") ||
         starts_with(text, "HTTP/1.0 401") ||
         starts_with(text, "HTTP/1.0 403") ||
         starts_with(text, "HTTP/1.0 404") ||
         starts_with(text, "HTTP/1.1 401") ||
         starts_with(text, "HTTP/1.1 403") ||
...
bool looks_like_rtcm3_stream(const std::vector<std::uint8_t> & buffer)
{
  ...
  const auto non_ascii_count = std::count_if(
    buffer.begin(),
    buffer.end(),
    [](std::uint8_t byte) {
      return byte == 0U || (byte < 0x09U) || (byte > 0x0DU && byte < 0x20U) || byte > 0x7EU;
    });
  ...
  return !starts_with(prefix, "HTTP/") &&
         !starts_with(prefix, "ICY ") &&
         !starts_with(prefix, "SOURCETABLE") &&
         !starts_with(prefix, "ERROR");
}
...
std::vector<std::uint8_t> extract_initial_payload(
  const std::vector<std::uint8_t> & buffer, const std::string & text_prefix)
{
  const auto header_end = text_prefix.find("\r\n\r\n");
  if (header_end != std::string::npos) {
    const auto payload_offset = header_end + 4U;
    if (payload_offset < buffer.size()) {
      return std::vector<std::uint8_t>(buffer.begin() + static_cast<std::ptrdiff_t>(payload_offset), buffer.end());
    }
    return {};
  }
...
void NtripConnection::start()
{
  ...
  boost::asio::post(io_context_, [this]() { schedule_resolve("starting", false); });
}
...
void NtripConnection::post_state(
  bool connected, const std::string & state, const std::string & error)
{
...
void NtripConnection::start_connect_timeout()
{
  ...
  connect_timer_.async_wait([self = shared_from_this()](const boost::system::error_code & ec) {
    ...
    [self = shared_from_this()](const boost::system::error_code & ec, std::size_t) {
      ...
      self->socket_.async_read_some(
        ...
        [self](const boost::system::error_code & read_ec, std::size_t bytes_read) {
          ...
          std::vector<std::uint8_t> initial_chunk(
            self->read_buffer_.begin(),
            self->read_buffer_.begin() + static_cast<std::ptrdiff_t>(bytes_read));
          ...
          if (is_textual_error_response(text_prefix) || is_textual_error_response(status_line)) {
            self->post_state(false, "caster_rejected", status_line.empty() ? text_prefix : status_line);
            self->schedule_resolve("reconnecting", true);
            return;
          }
          ...
          if (!is_success_status_line(status_line)) {
            self->post_state(
              false,
              "invalid_response",
              status_line.empty() ? "unrecognized handshake response" : status_line);
            self->schedule_resolve("reconnecting", true);
            return;
          }
...
void NtripConnection::start_read_watchdog()
{
  ...
  read_watchdog_.async_wait([self = shared_from_this()](const boost::system::error_code & ec) {
    if (ec || self->stopped_ || !self->handshake_complete_) {
      return;
    }
  ...
  socket_.async_read_some(
    ...
    [self = shared_from_this()](const boost::system::error_code & ec, std::size_t bytes_read) {
      ...
      if (callback && bytes_read > 0) {
        callback(std::vector<std::uint8_t>(
          self->read_buffer_.begin(),
          self->read_buffer_.begin() + static_cast<std::ptrdiff_t>(bytes_read)));
      }
